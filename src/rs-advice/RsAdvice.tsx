...
function ShowLove({ onAnswer }:{ onAnswer:(answer:string) => void }) {
    ...
    const nextHandler = () => {
        ...
        console.log('completed RS Advice... proceeding on');
        navigate('/my-rs-advice');
    }
    ...
    return <Stack sx={{
        ...
        <Box>
            <h1>👨‍🦳👵</h1>
            <h1>What's one RS advice you would give your younger self?</h1>
        </Box>
        ...
        <TextField value={response} onChange={(event) => setResponse(event.target.value)}
            ...
            id="rs-advice-textfield" label="Your response" variant="outlined" multiline rows={4}/>
